...
def update_weights(optimizer, network, batch, weight_decay, writer, iteration_n, num_unroll_steps, range_v, policy_w):
    loss = 0
    l_v = 0 
    l_p = 0
    l_r = 0
    ...
    for i in range(num_unroll_steps+1):
        ...
        l_p += binary_cross_entropy(F.softmax(policy_logits,dim=1), target_policy[:,i])*gradient_scale*policy_w
